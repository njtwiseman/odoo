...
class MobileServiceShop(models.Model):
    ...
    def action_send_mail(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('auto_service_shop', 'email_template_auto_service')[1]
        except ValueError:
            template_id = False
        try:
    ...
    def get_ticket(self):
        ...
        complaint_text = ""
        description_text = ""
        complaint_id = self.env['auto.complaint.tree'].search([('complaint_id', '=', self.id)])
        if complaint_id:
...
class MobileComplaintTypeTemplate(models.Model):
    ...
    complaint_type_template = fields.Many2one('auto.complaint', string="Complaint Type Template", required=True)
...
class MobileComplaintTree(models.Model):
    ...
    complaint_id = fields.Many2one('auto.service')
